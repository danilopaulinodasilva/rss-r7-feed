...
/*
Plugin Name: RSS Feed Portal R7 AWS
Description: Plugin para criar um feed RSS para o Portal R7 da Rede Record personalizado na hospedagem da Amazon Web Services
Version: 1.0.0
Author: Danilo P. da Silva
Author URI: https://dps.tec.br
Plug-in URI: https://github.com/danilopaulinodasilva/rss-r7-feed-aws
License: GPL-3.0+
...
function custom_feed_template() {
  ...
  if ($query->have_posts()) {
    while ($query->have_posts()) {
      ...
      $post_date_formatted = date('D, d M Y H:i:s +0000', strtotime($post_date));
      $date_object = new DateTime($post_date); // Cria um objeto DateTime com a data
      $post_date_formatted = $date_object->format('D, d M Y H:i:s +0000'); // Formata a data em inglês
      ...
      echo '<guid isPermaLink="false">' . $post_link . '</guid>';
      echo '<pubDate>' . $post_date_formatted . '</pubDate>';
